    ...
    <div className="p-4 space-y-4 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-bold flex items-center gap-2">
          <CandlestickChart className="w-5 h-5 text-primary" />交易所监控
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {statuses.map((s) => {
          ...
          return (
            <Card key={s.exchange} className={cn("p-4 border", s.connected ? "border-profit/30" : "border-border")}>
              {/* 标题 */}
                ...
                <div className="space-y-1 mb-3">
                  {bal.total_equity !== null && bal.total_equity !== undefined && (
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-bold tabular-nums">${(bal.total_equity || 0).toFixed(2)}</span>
                    </div>
                  )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead><tr className="text-muted-foreground border-b border-border">
                ...
                <th className="text-right py-2 px-2">标记价</th>
                ...
                {positions.map((p: any, i: number) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-border/30 hover:bg-muted/20">
                      ...
                      <td className="py-2 px-2 text-right tabular-nums text-muted-foreground">{(p.mark_price || 0).toLocaleString()}</td>
