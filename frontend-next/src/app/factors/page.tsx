...
export default function FactorsPage() {
  ...
  // 展示层派生值（仅供 Aurora 概览可视化，不改动数据接线）
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <PageHeader
        ...
        title="因子系统"
        subtitle="实时因子值 · 因子报告卡 · admission 门槛可视化"
        breadcrumb={[{ label: "市场 & 分析" }, { label: "因子系统" }]}
        refreshHint="评估任务按需触发"
        actions={
          <Button variant="outline" size="sm" className="btn-glow" onClick={() => load(symbol)} disabled={loading}>
      ...
      <Card className="p-3 glass">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="hidden sm:block w-px h-4 bg-border/60 mx-1" />
          <Button
                ...
                {overview.top.map((f) => (
                  <div key={f.name}>
                    ...
                    <div className="relative h-2 rounded-full bg-white/5">
                      ...
                      {f.normalized > 0 && (
                        <div
                          className="absolute left-1/2 top-0 bottom-0 rounded-r-full bg-profit/70"
                          style={{ width: `${Math.min(50, (f.normalized / overview.maxAbs) * 50)}%` }}
                        />
                      )}
                      {f.normalized < 0 && (
                        <div
                          className="absolute right-1/2 top-0 bottom-0 rounded-l-full bg-loss/70"
                          style={{ width: `${Math.min(50, (Math.abs(f.normalized) / overview.maxAbs) * 50)}%` }}
                        />
                      )}
                    </div>
                  </div>
              ...
              <div className="flex items-end gap-1.5 h-28">
                {overview.bins.map((c, i) => (
                  <div key={i} className="flex-1 h-full flex flex-col items-center justify-end gap-1">
                    ...
                    <div
                      className="w-full rounded-t-sm bg-gradient-to-t from-cyan-400/80 to-violet-500/80"
