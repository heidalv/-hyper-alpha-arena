...
以 **2026-08-19 仓库代码** 为准，不要拿 8 月中旬 SwingAgent 设计稿当现状。同日已做代码复核并修订文档。
...
| [`docs/因子挖掘回测全链路深度调查_20260820.md`](docs/因子挖掘回测全链路深度调查_20260820.md) | GP/MCTS/打分/晋升：算法公式、双池、与短线中线审查的交叉 |
| [`docs/因子挖掘算法升级与双卡本地LLM设计_20260820.md`](docs/因子挖掘算法升级与双卡本地LLM设计_20260820.md) | 挖掘算法评价、口径/目标升级、2×2080Ti 22G+NVLink 与本地 14B 分工（未改代码） |

代码默认：`MIDLONG_MID_VIA_FACTOR_ROUTE=false`（中线暂停）、`MIDLONG_REVIEW_LLM=false`（出场默认规则复查）、`SCALP_EV_GATE_ENABLED=true`。某日事实清单里的开关值不能当成此刻 `.env`。
