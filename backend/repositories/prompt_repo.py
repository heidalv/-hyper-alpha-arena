...
from backend.database.models import PromptTemplate, AccountPromptBinding, Account
...
def create_template(
    ...
    *,
    ...
    required_placeholders: Optional[list] = None,
    is_legacy: Optional[str] = None,
) -> PromptTemplate:
    template = PromptTemplate(
        ...
        is_legacy=is_legacy if is_legacy is not None else "true",
    )
    db.add(template)
    db.commit()
    db.refresh(template)
