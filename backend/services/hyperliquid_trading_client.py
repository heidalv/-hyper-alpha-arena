...
from backend.database.models import Account, HyperliquidExchangeAction
...
class HyperliquidTradingClient:
    ...
    def __init__(self, account_id: int, private_key: str, environment: str = "testnet", wallet_address: Optional[str] = None):
        ...
        import sys
        logger.info(f"[DEBUG __init__] account_id={account_id}, environment={environment}, wallet_address={wallet_address}", file=sys.stderr, flush=True)
    ...
    def _serialize_payload(self, payload: Optional[Any]) -> Optional[str]:
        ...
        try:
            ...
            logger.warning(f"Failed to serialize payload as JSON, falling back to str: {e}")
            try:
                return str(payload)
            except Exception:
    ...
    def _get_builder_params(self) -> Optional[Dict[str, Any]]:
        ...
        🔓 LOCAL UNLOCK: Builder fee DISABLED for local use.
        All trading is completely fee-free.

        Returns:
            None - No builder fees applied
        """
        # 🔓 DISABLED: No builder fees for local deployment
        return None
    ...
    def get_positions(self, db: Session, include_timing: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"=== CCXT RAW POSITIONS DATA ===")
            logger.info(positions_raw)
            logger.info(f"=== END CCXT RAW DATA ===")
    ...
    def place_order(
        ...
        # 同币已有仓：adopt 现有杠杆，避�?update_leverage 覆盖交易所统一�?        if not reduce_only:
            try:
                ...
                _adopt = extract_existing_symbol_leverage(symbol, _pos)
                ...
                logger.debug("[HLAdoptLev] skip: %s", _adopt_err)
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Set leverage to {leverage}x for {symbol} ({'cross' if is_cross else 'isolated'}), result: {result}")
                self._record_exchange_action(
                    action_type="set_leverage",
                    status="success",
                    symbol=symbol,
                    leverage=leverage,
                    request_payload={"symbol": symbol, "leverage": leverage, "is_cross": is_cross},
    ...
    def set_leverage(self, db: Session, symbol: str, leverage: int, is_cross: bool = False) -> bool:
        """
        ...
        Args:
            ...
            is_cross: True=cross margin, False=isolated margin (default isolated)
        """
        self._validate_environment(db)
        ...
        try:
            ...
            logger.info(f"Setting leverage for {symbol} to {leverage}x ({mode_str}) on {self.environment}")
    ...
    def get_tpsl_orders(self, db: Session, symbol: str) -> Dict[str, Optional[Dict[str, Any]]]:
        ...
        import sys
        logger.info(f"[TPSL DEBUG] {symbol} - Found {len(open_orders)} open orders", file=sys.stderr, flush=True)
        logger.info(f"[TPSL DEBUG] {symbol} - Found {len(open_orders)} open orders")
        for i, order in enumerate(open_orders):
            logger.info(f"[TPSL DEBUG] Order {i}: {order}", file=sys.stderr, flush=True)
    ...
    def update_tpsl(
        ...
        try:
            ...
            # ============================================================
            logger.info(f"[TPSL UPDATE] {symbol} - Fetching current orders from Hyperliquid API...", file=sys.stderr, flush=True)
            ...
            logger.info(f"[TPSL UPDATE] {symbol} - API returned: TP={current_tp_price}, SL={current_sl_price}", file=sys.stderr, flush=True)
            logger.info(f"[TPSL UPDATE] {symbol} - Requested: TP={new_tp_price}, SL={new_sl_price}", file=sys.stderr, flush=True)
            logger.info(f"[TPSL UPDATE] {symbol} - Found {len(all_tp_orders)} TP orders, {len(all_sl_orders)} SL orders", file=sys.stderr, flush=True)
            ...
            if new_tp_price is not None and current_tp_price is not None and current_tp_price > 0:
                ...
                tp_matches_existing = True
                logger.info(f"[TPSL UPDATE] {symbol} No new TP requested - SKIP", file=sys.stderr, flush=True)
            ...
            logger.info(f"[TPSL UPDATE] {symbol} - Update decision: TP_update={tp_needs_update}, SL_update={sl_needs_update}", file=sys.stderr, flush=True)
            ...
            if final_tp_price is not None or final_sl_price is not None:
                ...
                logger.info(f"[TPSL CACHE] {symbol} - Updated cache after operation: TP={final_tp_price}, SL={final_sl_price}", file=sys.stderr, flush=True)
    ...
    def get_order_status(self, db: Session, order_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 2. Check historical orders (fallback)
            history = self.sdk_info.historical_orders(self.wallet_address)
            for order in history:
                if order.get('oid') == order_id:
                    hl_status = order.get('status', '').lower()
                    # Map Hyperliquid status to standard status
                    status = 'filled' if hl_status == 'filled' else 'canceled' if hl_status == 'canceled' else hl_status
    ...
    def place_order_with_tpsl(
        ...
        """
        import sys
        logger.info(f"[DEBUG ENTRY] place_order_with_tpsl called: symbol={symbol}, price={price}, size={size}, TP={take_profit_price}, SL={stop_loss_price}", file=sys.stderr, flush=True)
        ...
        logger.info(f"[PRECISION] Fetching precision for {symbol}...", file=sys.stderr, flush=True)
        ...
        logger.info(f"[PRECISION] Original values - price: {price}, size: {size}, TP: {take_profit_price}, SL: {stop_loss_price}", file=sys.stderr, flush=True)
        ...
        logger.info(f"[PRECISION] Price adjusted: {original_price} -> {price}", file=sys.stderr, flush=True)
        ...
        logger.info(f"[PRECISION] Size adjusted: {original_size} -> {size}", file=sys.stderr, flush=True)
        ...
        if take_profit_price is not None:
            ...
            logger.info(f"[PRECISION] TP adjusted: {original_tp} -> {take_profit_price}", file=sys.stderr, flush=True)
        ...
        if stop_loss_price is not None:
            ...
            logger.info(f"[PRECISION] SL adjusted: {original_sl} -> {stop_loss_price}", file=sys.stderr, flush=True)
        ...
        try:
            # Set leverage before placing order (isolated margin by default)
            try:
                result = self.sdk_exchange.update_leverage(leverage, symbol, is_cross=is_cross)
                logger.debug(f"Set leverage to {leverage}x for {symbol} ({'cross' if is_cross else 'isolated'}), result: {result}")
                self._record_exchange_action(
            ...
            if status in ["filled", "resting"] and (take_profit_price or stop_loss_price):
                ...
                if take_profit_price:
                    try:
                        ...
                        # TP: use market trigger with slippage-protected limit price
                        # For long TP (sell): limit_px slightly below trigger to ensure fill
                        # For short TP (buy): limit_px slightly above trigger to ensure fill
                        tp_slippage = take_profit_price * 0.005  # 0.5% slippage protection
                        if is_buy:
                            tp_limit_px = take_profit_price - tp_slippage  # Sell below trigger
                        else:
                            tp_limit_px = take_profit_price + tp_slippage  # Buy above trigger
                        tp_limit_px = self._round_to_precision(
                            tp_limit_px, price_decimals, size_decimals,
                            is_price=True, price_tick=price_tick, size_step=size_step,
                        )
                ...
                if stop_loss_price:
                    try:
                        ...
                        # SL: use market trigger with slippage-protected limit price
                        # For long SL (sell): limit_px slightly below trigger to ensure fill
                        # For short SL (buy): limit_px slightly above trigger to ensure fill
                        sl_slippage = stop_loss_price * 0.005  # 0.5% slippage protection
                        if is_buy:
                            sl_limit_px = stop_loss_price - sl_slippage  # Sell below trigger
                        else:
                            sl_limit_px = stop_loss_price + sl_slippage  # Buy above trigger
                        sl_limit_px = self._round_to_precision(
                            sl_limit_px, price_decimals, size_decimals,
                            is_price=True, price_tick=price_tick, size_step=size_step,
                        )
            ...
            if status in ["filled", "resting"] and (take_profit_price or stop_loss_price):
                ...
                logger.info(f"[TPSL CACHE] {symbol} - Cached new TP/SL from place_order_with_tpsl: TP={take_profit_price}, SL={stop_loss_price}", file=sys.stderr, flush=True)
