...
class FactorBacktestScorer:
    ...
    def _load_klines(symbol: str, interval: str, limit: int):
        ...
        try:
            ...
            rows = kline_service.get_klines_from_db(
                symbol.upper(), interval, limit, exchange="hyperliquid",
            )
            return rows or None
        except Exception as e:
    ...
    def _walk_forward_backtest(
        ...
        # 年化 Sharpe（按 fwd 步长近似的样本，乘 sqrt(样本数) 做尺度）
        sharpe = float(mean_r / (std_r + 1e-12) * np.sqrt(len(arr))) if std_r > 0 else 0.0
    ...
    def _active_factor_series(
        ...
        try:
            ...
            if first is None:
                return out
            records = pool if pool is not None else custom_factor_store.list_active()
            for rec in records:
    ...
    def score_formula(
        ...
        *,
        ...
        fwd = int(fwd if fwd is not None else _cfg("FACTOR_SCORER_FWD_PERIOD", 5))
        ...
        for sym in syms:
            ...
            # 样本外回测
            bt = self._walk_forward_backtest(factor_vals, arrays["close"], fwd, cost)
            if bt["trades"] > 0:
        ...
        perf_ok = result.oos_sharpe >= min_sharpe and result.oos_net_return > min_net
    ...
    def validate_and_promote(self, factor_id: str) -> FactorScoreResult:
        ...
        rec = custom_factor_store.get(factor_id)
        if not rec:
        ...
        if _horizon == "midlong":
            ...
            result = self.score_formula(
                ...
                redundancy_pool=[
                    r for r in custom_factor_store.list_active()
    ...
    def validate_all_candidates(self, limit: int = 20) -> Dict[str, Any]:
        ...
        candidates = custom_factor_store.list_candidates()[:limit]
        results = []
