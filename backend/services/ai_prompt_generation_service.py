...
from backend.database.models import Account, AiPromptConversation, AiPromptMessage
...
class AiPromptGenerationService:
    """单次 Chat Completions：策略学习 / 提示词进化（account_id=None 时自动选活跃 AI 账户）。"""

    def __init__(self, db: Session):
        self._db = db

    def _resolve_llm_account(self, account_id: Optional[int]) -> Optional[Account]:
        base = self._db.query(Account).filter(Account.is_active == "true")
        if account_id is not None:
            acc = base.filter(Account.id == account_id).first()
        else:
            acc = (
                base.filter(Account.account_type == "AI")
                .order_by(Account.id.asc())
                .first()
            )
            if not acc:
                acc = base.order_by(Account.id.asc()).first()
        if not acc:
            logger.warning("[AiPromptGen] 无可用账户，无法调用 LLM")
            return None
        from backend.services.ai_decision_service import resolve_account_llm_config

        resolve_account_llm_config(self._db, acc)
        if not acc.api_key or not (acc.base_url or "").strip():
            logger.warning(
                "[AiPromptGen] 账户 '%s' 缺少 api_key 或 base_url",
                getattr(acc, "name", acc.id),
            )
            return None
        return acc
    ...
    def generate_with_conversation(
        ...
        *,
        ...
        """执行一次补全；若有 ```prompt 块则抽取，否则使用全文。"""
        acc = self._resolve_llm_account(account_id)
        if not acc or not messages:
            return None
        ...
        for endpoint in endpoints:
            try:
                ...
                if response.status_code == 200:
                    break
                logger.warning(
                    "[AiPromptGen] %s → HTTP %s %s",
                    endpoint,
                    response.status_code,
                    (response.text or "")[:240],
                )
            except Exception as e:
                logger.warning("[AiPromptGen] 请求异常 %s: %s", endpoint, e)
