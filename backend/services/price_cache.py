...
class PriceCache:
    ...
    def __init__(self, ttl_seconds: int = 3, history_seconds: int = 3600):
        # key: (symbol, market, environment, exchange), value: (price, timestamp)
        self.cache: Dict[Tuple[str, ...], Tuple[float, float]] = {}
        # key: (symbol, market, environment, exchange), deque of (timestamp, price)
        self.history: Dict[Tuple[str, ...], Deque[Tuple[float, float]]] = {}
        self.ttl_seconds = ttl_seconds
        self.history_seconds = history_seconds
        self.lock = Lock()

    def get(self, symbol: str, market: str, environment: str = "mainnet", exchange: str = "") -> Optional[float]:
        """Get cached price if still within TTL."""
        key = (symbol, market, environment, exchange)
    ...
    def record(self, symbol: str, market: str, price: float, timestamp: Optional[float] = None, environment: str = "mainnet", exchange: str = "") -> None:
    ...
    def get_history(self, symbol: str, market: str, environment: str = "mainnet", exchange: str = "") -> List[Tuple[float, float]]:
...
# TTL 覆盖 Hub stale watchdog 间隔（默认 30s）+ 缓冲
