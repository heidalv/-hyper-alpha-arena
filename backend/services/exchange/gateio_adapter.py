"""
GateioAdapter — Gate.io 交易所适配器

使用 CcxtBaseAdapter 共享基类，CCXT gateio 驱动。

激励特色:
- 持 GT 可降费
- Market Maker 负费率 -0.012%
- Startup 新币认购
"""
...
class GateioAdapter(CcxtBaseAdapter):
    """Gate.io 合约 + 现货适配器"""

    _ccxt_id = "gateio"
    _exchange_type = ExchangeType.GATEIO
    ...
    # Gate.io MM rebate rate (industry-leading)
    _MM_MAKER_REBATE = -0.00012  # -0.012%

    async def get_fee_tier(self) -> ExchangeFeeTier:
        """获取Gate.io费率 — 使用 batch fee query (v4.39.0+)"""
        cache = self._get_cache()
        cache_key = "gateio_fee_tier"
        ...
        try:
            # CCXT unified method for Gate.io
            ...
            if isinstance(result, dict):
                ...
                # Gate.io MM has -0.012% maker (industry-leading rebate)
                ...
                logger.info(
                    "[Gate.io] Fee tier fetched: %s maker=%.5f%% taker=%.5f%%",
                    tier_name, effective_maker * 100, taker * 100
                )
                return tier
        except Exception as e:
            logger.warning("GateioAdapter.get_fee_tier failed: %s", e)
    ...
    async def get_points_snapshot(self) -> ExchangePointsSnapshot:
        """Gate.io GT 持仓积分"""
        cache = self._get_cache()
        cache_key = "gateio_points"
        cached = cache.get(cache_key)
        if cached is not None:
            return cached

        snapshot = ExchangePointsSnapshot(exchange="gateio")

        if self._exchange is not None:
            try:
                # Check GT balance as proxy for fee discount eligibility
                bal = await self._exchange.fetch_balance()
                gt_balance = float(bal.get("total", {}).get("GT", 0) or 0)
                if gt_balance > 0:
                    snapshot = ExchangePointsSnapshot(
                        exchange="gateio",
                        points_balance=gt_balance,
                        points_multiplier=1.0,
                    )
            except Exception as e:
                logger.debug("GateioAdapter.get_points_snapshot: %s", e)
    ...
    async def get_rebate_info(self) -> ExchangeRebateInfo:
        """Gate.io返利 — MM级别有 -0.012% 返利"""
        cache = self._get_cache()
        cache_key = "gateio_rebate_info"
        cached = cache.get(cache_key)
        if cached is not None:
            return cached
    ...
    async def get_active_campaigns(self) -> List[Dict]:
        """Gate.io Startup等活动"""
        cache = self._get_cache()
        cache_key = "gateio_campaigns"
        cached = cache.get(cache_key)
        if cached is not None:
            return cached

        campaigns = [{
            "campaign_id": "gateio_startup",
