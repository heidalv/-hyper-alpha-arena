...
from backend.database.models import (
    Order, Position, Trade, User,
    CRYPTO_MIN_COMMISSION, CRYPTO_COMMISSION_RATE,
    CRYPTO_MIN_ORDER_QUANTITY, CRYPTO_LOT_SIZE,
)
...
def place_and_execute(db: Session, user: User, symbol: str, name: str, market: str, side: str, order_type: str, price: Optional[float], quantity: int) -> Order:
    # Only support CRYPTO market
    if market != "CRYPTO":
        raise ValueError("Only CRYPTO market is supported")

    # Adjust quantity to lot size
    if quantity % CRYPTO_LOT_SIZE != 0:
        raise ValueError(f"quantity must be a multiple of lot_size={CRYPTO_LOT_SIZE}")
    if quantity < CRYPTO_MIN_ORDER_QUANTITY:
        raise ValueError(f"quantity must be >= min_order_quantity={CRYPTO_MIN_ORDER_QUANTITY}")
