...
from backend.database.models import User, Order, Account
from backend.schemas.order import OrderCreate, OrderOut
from backend.services.order_matching import create_order, check_and_execute_order, get_pending_orders, cancel_order, process_all_pending_orders
from backend.repositories.user_repo import verify_user_password, user_has_password, set_user_password, verify_auth_session
...
class OrderCreateRequest(BaseModel):
    ...
    algo: Optional[str] = None  # 阶段 3.2: 执行算法（MARKET/TWAP/POV/FUNDING_IS/SOR）
    algo_config: Optional[dict] = None  # 算法参数（如 twap_slices）
...
async def create_new_order(request: OrderCreateRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if request.algo and request.algo.upper() != "MARKET":
            logger.info(
                f"User {user.username} created order: {order.order_no} "
                f"algo={request.algo} config={request.algo_config} "
