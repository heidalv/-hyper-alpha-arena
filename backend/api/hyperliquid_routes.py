...
from backend.database.models import HyperliquidExchangeAction
from backend.services.hyperliquid_environment import (
...
class HyperliquidSetupRequest(BaseModel):
    ...
    max_leverage: int = Field(20, ge=1, le=50, description="Maximum allowed leverage", alias="maxLeverage")
    default_leverage: int = Field(10, ge=1, le=50, description="Default leverage for orders", alias="defaultLeverage")
...
async def get_balance(
    ...
    try:
        ...
        # Blocking API call -> run in thread to prevent event loop freeze
        import asyncio
        balance = await asyncio.to_thread(client.get_account_state, db)
        balance["source"] = "live"
...
async def get_positions(
    ...
    try:
        ...
        # Blocking API call -> run in thread to prevent event loop freeze
        import asyncio
        positions = await asyncio.to_thread(client.get_positions, db)
        return {
            'account_id': account_id,
...
async def test_connection(
    ...
    try:
        ...
        # Blocking API call -> run in thread to prevent event loop freeze
        import asyncio
        result = await asyncio.to_thread(client.test_connection, db)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
...
class WalletConfigRequest(BaseModel):
    ...
    max_leverage: int = Field(20, ge=1, le=50, description="Maximum allowed leverage", alias="maxLeverage")
    default_leverage: int = Field(10, ge=1, le=50, description="Default leverage", alias="defaultLeverage")
            ...
            if request.environment == 'mainnet':
                try:
                    ...
                    if max_fee < HYPERLIQUID_BUILDER_CONFIG.builder_fee:
                        logger.info(f"[BUILDER_AUTH] Not authorized (max_fee={max_fee} < required={HYPERLIQUID_BUILDER_CONFIG.builder_fee}), triggering authorization")
                        ...
                        if is_success:
                            logger.info(f"[BUILDER_AUTH] Authorization completed for account {account_id}: {result}")
                        else:
                            logger.info(f"[BUILDER_AUTH] Authorization FAILED for account {account_id}: {result}")
                            requires_auth = True
                    else:
                        logger.info(f"[BUILDER_AUTH] Already authorized for account {account_id} (max_fee={max_fee})")
                except Exception as e:
                    logger.info(f"[BUILDER_AUTH] Authorization failed for account {account_id}: {type(e).__name__}: {e}")
                    requires_auth = True
