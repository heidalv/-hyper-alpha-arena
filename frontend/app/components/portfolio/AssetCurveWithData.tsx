...
const CACHE_TTL_MS = 5 * 60 * 1000 // 5 minutes — evict entries older than this
...
export default function AssetCurve({
  ...
  const storeCache = useCallback((tf: Timeframe, nextData: AssetCurveData[]) => {
    ...
    // Evict stale entries exceeding TTL
    const now = Date.now()
    cacheRef.forEach((entry, key) => {
      if (now - entry.lastFetched > CACHE_TTL_MS) cacheRef.delete(key)
    })
