...
export default function SystemLogs() {
  ...
  const [fileLogLines, setFileLogLines] = useState<string[]>([])
  const [fileLogPath, setFileLogPath] = useState<string>('')

  const fetchFileLogTail = async () => {
    try {
      const response = await fetch('/api/opencode/health/log-tail?lines=200')
      const data = await response.json()
      setFileLogLines(data.lines || [])
      setFileLogPath(data.path || '')
    } catch (error) {
      console.error('Failed to fetch file log tail:', error)
      toast.error('无法读取 backend.log')
    }
  }
  ...
  useEffect(() => {
    ...
    } else if (activeTab === 'filelog') {
      fetchFileLogTail()
    }
  }, [selectedCategory, selectedLevel, activeTab])

  // 自动刷新：每5秒刷新一次当前激活的标签页数据
  useEffect(() => {
    ...
    <div className="container mx-auto p-4 space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="filelog">文件日志</TabsTrigger>
        ...
        <TabsContent value="filelog" className="space-y-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="text-sm">backend.log 尾部（200 行）</CardTitle>
              <Button size="sm" variant="outline" onClick={fetchFileLogTail}>
                <RefreshCw className="w-4 h-4 mr-1" /> 刷新
              </Button>
            </CardHeader>
            <CardContent>
              {fileLogPath && (
                <p className="text-xs text-muted-foreground mb-2 font-mono truncate">{fileLogPath}</p>
              )}
              <ScrollArea className="h-[600px] pr-4">
