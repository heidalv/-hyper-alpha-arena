...
# Paper positions closed in last 2 days + still open
cur.execute("""
    SELECT id, symbol, side, entry_price, close_price, margin, unrealized_pnl,
           leverage, timeframe_tier, trade_nature, close_reason, status,
           partial_realized_pnl, partial_fee_paid, opened_at, closed_at,
           reduce_count, original_margin, tp_price, sl_price
...
for p in closed_pos:
    ...
    if net_pnl > 0:
        wins.append(net_pnl)
        win_details.append((p, net_pnl))
    else:
        losses.append(net_pnl)
        loss_details.append((p, net_pnl))
...
print(f"  盈亏比(Profit Factor): {total_win/total_loss:.2f}")
...
# Show top 5 wins and top 5 losses
print(f"\n--- TOP 5 盈利订单 ---")
for p, pnl in sorted(win_details, key=lambda x: -x[1])[:5]:
    print(f"  {p['symbol']:10s} {p['side']:5s} entry={p['entry_price']} close={p['close_price']} "
          f"net={pnl:+.2f} tier={p['timeframe_tier']} reason={p['close_reason']}")

print(f"\n--- TOP 5 亏损订单 ---")
for p, pnl in sorted(loss_details, key=lambda x: x[1])[:5]:
    print(f"  {p['symbol']:10s} {p['side']:5s} entry={p['entry_price']} close={p['close_price']} "
          f"net={pnl:+.2f} tier={p['timeframe_tier']} reason={p['close_reason']}")
...
if open_pos:
    print(f"\n--- 未平仓浮动 ---")
    total_unreal = sum(float(p['unrealized_pnl'] or 0) for p in open_pos)
    total_fee_open = sum(float(p['partial_fee_paid'] or 0) for p in open_pos)
    print(f"  浮动盈亏合计: {total_unreal:+.2f} USDT")
    print(f"  已付手续费: {total_fee_open:.2f} USDT")
    for p in open_pos[:10]:
        ...
        try:
            ...
            mark = '?'
        print(f"  {p['symbol']:10s} {p['side']:5s} entry={p['entry_price']} mark={mark} "
              f"unreal={p['unrealized_pnl']} tier={p['timeframe_tier']} margin={p['margin']}")
...
for p in reduced[:15]:
    ...
    net = partial_pnl - partial_fee
    print(f"  {p['symbol']:10s} side={p['side']:5s} reduce={p['reduce_count']} "
          f"partial_pnl={partial_pnl:+.2f} fee={partial_fee:.2f} net={net:+.2f} "
...
for o in reduce_orders[:15]:
    ...
    print(f"  {str(o['created_at'])[:19]:20s} {o['symbol']:10s} {o['side']:5s} "
          ...
          f"pnl={pnl:+.2f} fee={fee:.2f} net={net:+.2f} reason={o['close_reason']}")

# Reduce then re-open pattern
print(f"\n--- 减仓后重新开仓检测 ---")
cur.execute("""
    SELECT symbol, side, order_type, filled_price, quantity, filled_quantity,
           fee, pnl, close_reason, status, created_at
    FROM paper_orders
    WHERE created_at >= ?
...
for o in all_orders:
    sym = o['symbol']
    ts = o['created_at']
    if o['close_reason'] and ('partial' in str(o['close_reason']) or 'reduce' in str(o['close_reason'])):
        last_reduce_time[sym] = ts
    elif o['close_reason'] is None or o['close_reason'] == '':
        # Opening order
        if sym in last_reduce_time:
            try:
                lt = datetime.fromisoformat(last_reduce_time[sym].replace('Z','+00:00').replace(' ','T'))
                ct = datetime.fromisoformat(ts.replace('Z','+00:00').replace(' ','T'))
                gap = (ct - lt).total_seconds() / 60
                if gap < 60:  # within 60 minutes
                    reduce_then_open += 1
                    print(f"  减仓后{gap:.0f}分钟重开: {sym} at {str(ts)[:19]}")
            except:
                pass
print(f"  减仓后60分钟内重新开仓: {reduce_then_open}次")
...
for r in cur.fetchall():
    ...
    print(f"  tier={str(r['timeframe_tier']):6s} status={r['status']:10s} count={r['cnt']} margin={margin:.2f}")
...
""")
for r in cur.fetchall():
    print(f"  tier={str(r['timeframe_tier']):6s} active={r['cnt']}")

# All strategies by tier
print("\n--- 全部策略 tier 分布 ---")
cur.execute("""
    SELECT timeframe_tier, status, COUNT(*) as cnt FROM ai_strategies
    GROUP BY timeframe_tier, status
    ORDER BY status, timeframe_tier
""")
for r in cur.fetchall():
    print(f"  tier={str(r['timeframe_tier']):6s} status={r['status']:10s} count={r['cnt']}")
...
for m in memories[:10]:
    wr = m['win_rate'] or 0
    ap = m['avg_profit'] or 0
    al = m['avg_loss'] or 0
    print(f"  sid={m['strategy_id'][:20]:20s} trades={m['total_trades']} "
          f"win_rate={wr:.0%} avg_profit={ap:.2f} avg_loss={al:.2f}")

# Strategy trades
cur.execute("SELECT COUNT(*) as cnt FROM strategy_trades")
print(f"\n  策略交易明细数: {cur.fetchone()['cnt']}")

# How many strategies were reused
cur.execute("""
...
# Strategies created per symbol
print("\n--- 各symbol策略创建统计 ---")
cur.execute("""
    SELECT primary_symbol, auto_mode, COUNT(*) as cnt
    FROM ai_strategies
    GROUP BY primary_symbol, auto_mode
    ORDER BY primary_symbol, auto_mode
""")
for r in cur.fetchall():
    print(f"  symbol={r['primary_symbol']:10s} mode={r['auto_mode']:10s} count={r['cnt']}")
...
cur.execute("""
    SELECT symbol, side, entry_price, close_price, unrealized_pnl,
           partial_realized_pnl, partial_fee_paid, reduce_count, close_reason,
           opened_at, closed_at, timeframe_tier, margin
    FROM paper_positions
...
if hold_times:
    ...
    long_trades = [h for h in hold_times if h[0] > 1440]  # >24h
    print(f"  平均持仓时间: {avg_hold:.0f} 分钟 ({avg_hold/60:.1f} 小时)")
    print(f"  持仓<30分钟的: {len(short_trades)}/{len(hold_times)}")
    print(f"  持仓>24小时的: {len(long_trades)}/{len(hold_times)}")
    for h, t in short_trades[:10]:
        pnl = float(t['unrealized_pnl'] or 0) + float(t['partial_realized_pnl'] or 0)
        fee = float(t['partial_fee_paid'] or 0)
        net = pnl - fee
        print(f"    {t['symbol']:10s} {h:.0f}min net={net:+.2f} fee={fee:.2f} reason={t['close_reason']} tier={t['timeframe_tier']}")

# ═══════════════════════════════════════════════════════
# 6. 订单时间线
# ═══════════════════════════════════════════════════════
print(f"\n{'=' * 70}")
print("6. 近2天完整订单时间线 (最近50笔)")
print("=" * 70)
for o in orders[:50]:
...
# Per-symbol trade frequency
sym_counts = defaultdict(int)
for o in orders:
    if o['close_reason'] is None or o['close_reason'] == '':
        sym_counts[o['symbol']] += 1

print("\n--- 各币种开仓频率 ---")
for sym, cnt in sorted(sym_counts.items(), key=lambda x: -x[1]):
    print(f"  {sym:10s}: {cnt}次开仓")

# Per-day trade count
day_counts = defaultdict(int)
for o in orders:
    ts = str(o['created_at'] or '')[:10]
    if ts:
        day_counts[ts] += 1
...
# ═══════════════════════════════════════════════════════
print(f"\n{'=' * 70}")
print("8. 策略基因(genome)分析")
print("=" * 70)

cur.execute("""
    SELECT strategy_id, primary_symbol, timeframe_tier, status, genome
    FROM ai_strategies
    WHERE status = 'active'
    ORDER BY primary_symbol, timeframe_tier
""")
active_strats = cur.fetchall()
print(f"\n活跃策略数: {len(active_strats)}")

nature_counts = defaultdict(int)
for s in active_strats:
    genome = json.loads(s['genome'] or '{}')
    nature = genome.get('trade_nature', 'NOT_SET')
    nature_counts[nature] += 1

print("\n--- 活跃策略 genome.trade_nature ---")
for n, cnt in sorted(nature_counts.items(), key=lambda x: -x[1]):
    print(f"  {n}: {cnt}")
